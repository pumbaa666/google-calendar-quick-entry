...
public class JDayChooser extends JPanel implements ActionListener, KeyListener, FocusListener
{
	...
	protected JValueButton[] days;

	protected JButton[] weeks;

	protected JValueButton selectedDay;
	...
	{
		return this.calendar.getMaximum(Calendar.DATE);
	}
	...
	public JDayChooser(boolean weekOfYearVisible)
	{
		...
		for(int y = 0; y < 7; y++)
		{
			for(int x = 0; x < 7; x++)
			{
				...
				days[index].setHorizontalAlignment(SwingConstants.LEFT);
...
//			days[k].setValue(0);
	...
	public void setDay(int d)
	{
		...
		if(selectedDay != null)
		{
			...
			selectedDay.repaint();
			selectedDay.refreshBackground();
		}

		for(int i = 7; i < 49; i++)
		{
			if(days[i].getValue() == day)
			{
				selectedDay = days[i];
...
//	}
	...
	class DecoratorButton extends JValueButton
	{
